...
from cqrs_ddd_auth.ports.communication import EmailSenderPort, SMSSenderPort
...
class EmailOTPService:
    ...
    async def send_challenge(self, claims: UserClaims, method: str) -> str:
        ...
        if not self.otp_repo or not self.email_sender:
            raise RuntimeError("Email OTP not configured")
    ...
    async def validate(self, claims: UserClaims, method: str, code: str) -> bool:
        ...
        totp = pyotp.TOTP(
            challenge.secret, 
            digits=len(code), 
            interval=self.expiration_seconds
        )
...
class SMSOTPService:
    ...
    async def send_challenge(self, claims: UserClaims, method: str) -> str:
        ...
        if not self.otp_repo or not self.sms_sender:
            raise RuntimeError("SMS OTP not configured")
        ...
        await self.sms_sender.send(
            to=phone,
            message=f"{self.app_name}: Your verification code is {code}"
        )
...
class CompositeOTPService:
    ...
    def _get_service(self, method: str):
        ...
        if not service:
            raise ValueError(f"OTP method '{method}' not configured")
        return service
